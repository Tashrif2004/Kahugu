...
class KahuguEngine {
    ...
    private val systemPrompt = """
        ...
        Kuwa mfupi, wa moja kwa moja, na wa kitaalamu - kama msaidizi
        wa kweli, sio chatbot ya jumla. Ukikutana na ombi la kufanya
        kitendo nje ya app, SEMA WAZI kuwa unahitaji ruhusa ya wazi
        ya mtumiaji kabla ya kutekeleza.
    """.trimIndent()
            ...
            client.newCall(request).execute().use { response ->
                ...
                if (!response.isSuccessful) {
                    return@withContext Result.failure(
                        IOException("Gemini API error ${response.code}: $responseBody")
                    )
                }
